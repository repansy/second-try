"""
brief: for depth
path:input: left_017,right.png
    output: ./data/check_rectification.png
            ./data/disaprity.png
"""
...
def getDepthMapWithConfig(disparityMap: np.ndarray, config: stereoconfig.stereoCamera) -> np.ndarray:
    fb = config.cam_matrix_left[0, 0] * config.T[0]     # 基线 x 焦距
    doffs = config.doffs
...
if __name__ == '__main__':
    # 读取MiddleBurry数据集的图片
    iml = cv2.imread('G:\\2023-2024-1\\VR_exp\\4\\py_self\\left_017.png', 1)  # 左图
    imr = cv2.imread('G:\\2023-2024-1\\VR_exp\\4\\py_self\\right_017.png', 1)  # 右图
    ...
    disp_l, disp_r = stereoMatchSGBM(iml_, imr_, False)  # 这里传入的是未经立体校正的图像，因为我们使用的middleburry图片已经是校正过的了
    # cv2.imwrite('./data/disaprity.png', disp * 4)

    # 计算深度图
    # depthMap = getDepthMapWithQ(disp, Q)
    depthMap = getDepthMapWithConfig(disp_l, config)  # 求深度图
    minDepth = np.min(depthMap)
    maxDepth = np.max(depthMap)
    print(minDepth, maxDepth)
    depthMapVis = (255.0 * (depthMap - minDepth)) / (maxDepth - minDepth)  # 归一化处理
    depthMapVis = depthMapVis.astype(np.uint8)  # 感觉没必要？
